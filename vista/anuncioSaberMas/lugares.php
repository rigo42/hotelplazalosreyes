<div class="col-md-12 col-lg-4 sidebar">
  <div class="sidebar-box">
    <h3 class="heading">Lugares recomendados a visitar</h3>
    <div class="post-entry-sidebar">
      <ul class="seleccionAnuncio">
        <li>
          <a href="<?php echo URL ?>viajeros/sanBlas">
            <img src="<?php echo URL ?>libreria/images/anuncioSanblasPrincipalMin.jpg" alt="Image placeholder" class="mr-4">
            <div class="text">
              <h4>San Blas</h4>
              <div class="post-meta">
                <span class="mr-2">Fecha: <script>document.write(new Date().getFullYear());</script></span> &bullet;
              </div>
            </div>
          </a>
        </li>
        <li>
          <a href="<?php echo URL ?>viajeros/mexcaltitan">
            <img src="<?php echo URL ?>libreria/images/anuncioMexcaltitanPrincipalMin.jpg" alt="Image placeholder" class="mr-4">
            <div class="text">
              <h4>Mexcaltitán pueblo magico</h4>
              <div class="post-meta">
                <span class="mr-2">Fecha: <script>document.write(new Date().getFullYear());</script></span> &bullet;
              </div>
            </div>
          </a>
        </li>
        <li>
          <a href="<?php echo URL ?>viajeros">
            <img src="<?php echo URL ?>libreria/images/anuncioViajes2Principal.jpg" alt="Image placeholder" class="mr-4">
            <div class="text">
              <h4>Como ser un buen viajero</h4>
              <div class="post-meta">
                <span class="mr-2">Fecha: <script>document.write(new Date().getFullYear());</script></span> &bullet;
              </div>
            </div>
          </a>
        </li>
      </ul>
    </div>
  </div>
</div>